...
type LoginSessionInfo = Express.User
...
export const getLoginSessionInfoById = async (loginSessionId: string) => {
    const loginSessionInfoJson = await redisClient.get(`LOGIN_SESSION:${loginSessionId}`);
    if (loginSessionInfoJson) {
        ...
        const user = await prisma.loginSession.findUnique({
            where: {
                id: loginSessionId
            }
        }).user();
